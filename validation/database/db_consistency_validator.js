...
function runDBConsistencyCheck(db) {
  ...
  try {
    ...
    const orphanRelations = db.prepare(`
      ...
      LEFT JOIN structure_events p ON r.parent_event_id = p.event_id
      LEFT JOIN structure_events c ON r.child_event_id = c.event_id
    ...
    const orphanContexts = db.prepare(`
      ...
      LEFT JOIN structure_events e ON c.event_id = e.event_id
