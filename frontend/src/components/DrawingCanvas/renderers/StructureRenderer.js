...
export default class StructureRenderer {
  ...
  project(event, utils) {
    ...
    const ryLevel = Math.round(ry0 + rh / 2);
  ...
  draw(ctx, projected, config, isSelected) {
    ...
    const { rx0, ry0, rx1, ry1, rw, rh, ryLevel, event, debugMode } = projected;
    ...
    if (debugMode) {
      ...
      ctx.setLineDash([]);
      ctx.beginPath();
      ctx.moveTo(rx1, ryLevel - 8);
      ctx.lineTo(rx1, ryLevel + 8);
      ctx.stroke();
      ...
      ctx.textAlign = 'left';
      ctx.fillText(event.type.toUpperCase(), rx0 + 5, ryLevel - 2);
      ctx.restore();
      return;
    }
    ...
    if (validationStatus) {
      ...
      labelText = config.labelText || event.type.toUpperCase();
    }
