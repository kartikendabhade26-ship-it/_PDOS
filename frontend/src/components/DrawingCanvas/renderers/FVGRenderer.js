...
export default class FVGRenderer {
  renderType = 'interval';
  layer = 'ZonesLayer';
  ...
  draw(ctx, projected, config, isSelected) {
    ...
    // Fill background for other zones (e.g. FVGs, OBs)
    ctx.save();
    ...
    if (!validationStatus && !event.outcome) {
      ctx.setLineDash([4, 4]); // dashed border for unlabeled/unscanned candidates
    } else {
      ctx.setLineDash([]);
    }
    ctx.beginPath();
    ...
    // Draw midline if selected or if outcome is bounce_50pct
    ...
    if (validationStatus) {
      ...
      labelText = config.labelText || (event.direction === 'bullish' ? 'BISI' : 'SIBI');
    }
