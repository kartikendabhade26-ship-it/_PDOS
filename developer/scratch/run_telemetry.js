// developer/scratch/run_telemetry.js
const fs = require('fs');
...
async function main() {
  ...
  console.log(`Parsed ${bars.length} bars. Cleaning DB and starting sync symbol pipeline...`);
  ...
  await syncSymbolPipeline('NQ_Historical_Data', bars, runId);
  console.log("Telemetry run completed successfully.");
}

main().catch(err => {
  console.error("Telemetry run failed:", err);
  process.exit(1);
});
